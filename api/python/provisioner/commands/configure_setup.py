...
class ConfigureSetup(CommandParserFillerMixin):
    ...
    def _list_to_dict(self, list, last_node_val: None):
        """Recursively convert a list to a nested dictionary
        
        list: List to be converted to a Python dict
        last_node_val: value to be assigned to last node in tree.
        """
        new_dict = dict()
        if len(list) > 1:
            # Recurse
            new_dict[list[0]] = self._list_to_dict(
                list[1:],
                last_node_val
            )
        else:
            new_dict[list[0]] = last_node_val
